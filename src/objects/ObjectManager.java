...
public class ObjectManager {
    ...
    public void checkKeyTouched(Player p, int xLvlOffset){
        if (currentLevel.getKey().getHitbox().intersects(p.getHitbox())
                && !currentLevel.getKey().isFollowPlayer())
            currentLevel.getKey().followPlayer(p, xLvlOffset);
        else if (currentLevel.getKey().isFollowPlayer())
            currentLevel.getKey().followPlayer(p, xLvlOffset);
    }

    public void checkDoorTouched(Key key){
        if (currentLevel.getDoor().getHitbox().intersects(key.getHitbox())){
            currentLevel.getDoor().setActive(false);
            currentLevel.getKey().setActive(false);
    ...
    private void drawDoor(Graphics g, int xLvlOffset) {
        ...
        if(!currentLevel.getDoor().isActive())
            temp = doorOpen;

        g.drawImage(temp, (int) (currentLevel.getDoor().getHitbox().x - xLvlOffset),
                (int) (currentLevel.getDoor().getHitbox().y - currentLevel.getDoor().getyDrawOffset() - 1),
    ...
    private void drawKey(Graphics g, int xLvlOffset){
        if (currentLevel.getKey().isActive()){
            g.drawImage(keyImg, (int) (currentLevel.getKey().getHitbox().x - xLvlOffset),
                    (int) (currentLevel.getKey().getHitbox().y - currentLevel.getKey()
                            .getyDrawOffset()),
