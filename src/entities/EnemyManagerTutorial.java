...
public class EnemyManagerTutorial {

    private Tutorial tutorial;
    private BufferedImage[][] crabbyArr, pinkstarArr, sharkArr;
    private TutLevel currentLevel;

    public EnemyManagerTutorial(Tutorial tutorial) {
        this.tutorial = tutorial;
        loadEnemyImgs();
    }

    public void loadEnemies(TutLevel level) {
    ...
    public void update(int[][] lvlData) {
        ...
        for (Crabby c : currentLevel.getCrabs())
            if (c.isActive()) {
                c.update(lvlData, tutorial);
                isAnyActive = true;
            }

        for (Pinkstar p : currentLevel.getPinkstars())
            if (p.isActive()) {
                p.update(lvlData, tutorial);
                isAnyActive = true;
            }

        for (Shark s : currentLevel.getSharks())
            if (s.isActive()) {
                s.update(lvlData, tutorial);
                isAnyActive = true;
            }
