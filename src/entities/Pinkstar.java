...
public class Pinkstar extends Enemy {
            ...
            switch (state) {
                ...
                case RUNNING:
                    if (canSeePlayer(lvlData, tutorial.getPlayer())) {
                        newState(ATTACK);
                        setWalkDir(tutorial.getPlayer());
                    }
                    move(lvlData, tutorial);
                    break;
                case ATTACK:
                    if (preRoll) {
                        if (aniIndex >= 3)
                        ...
                        move(lvlData, tutorial);
                        checkDmgToPlayer(tutorial.getPlayer());
                        checkRollOver(tutorial);
                    }
                    break;
                case HIT:
                    if (aniIndex <= GetSpriteAmount(enemyType, state) - 2)
    ...
    private void checkDmgToPlayer(PlayerTutorial player) {
        if (hitbox.intersects(player.getHitbox()))
            if (tickSinceLastDmgToPlayer >= 60) {
                tickSinceLastDmgToPlayer = 0;
                player.changeHealth(-GetEnemyDmg(enemyType), this);
    ...
    private void setWalkDir(PlayerTutorial player) {
        if (player.getHitbox().x > hitbox.x)
            walkDir = RIGHT;
        else
            walkDir = LEFT;
    ...
    protected void move(int[][] lvlData, Tutorial tutorial) {
        float xSpeed = 0;
    ...
    private void checkRollOver(Tutorial tutorial) {
        rollDurationTick++;
        if (rollDurationTick >= rollDuration) {
            rollOver(tutorial);
            rollDurationTick = 0;
    ...
    private void rollOver(Tutorial tutorial) {
        newState(IDLE);
        tutorial.addDialogue((int) hitbox.x, (int) hitbox.y, QUESTION);
    }
